...
def getColorNms ():
    ...
    curs = conn.cursor()
    sql = "select attr_no||'_'||replace(attr_info, ' ', '-'), attr_no from ONETEN.ATTR_M where attr_grp_no = 3 and use_yn = 'Y' and attr_no <> 170 order by sort_no  "
    curs.execute(sql)
...
def getColorPrdImgList(attr_no):
    ...
    sql = "select  'http://1ten-image.wishlink.net/llbt/64x64/src' || b.img_url || '?dummy=20170524', c.filename from \
            ...
            where a.reg_dtm >= to_date('20180101000000', 'yyyymmddhh24miss') and  a.prd_use_flag = '02' AND a.prd_disp_flag not in ('01' ) and  mod(a.ot_prd_no,10) > 0   "
