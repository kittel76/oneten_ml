...
class ContentEngine(object):
    ...
    def train(self, data_source):
        ...
        logger.info("Training data ingested in %s seconds.", (time.time() - start))
    ...
    def _train(self, ds):
        ...
        for idx, row in ds.iterrows():
            similar_indices = cosine_similarities[idx].argsort()[:-42:-1]
            ...
            # print("row['id']", row['id'], [ ds['id'][i] for i in similar_indices[1:]], similar_indices)
            dbio.insert_related_prd_list_tmp(row['id'], [ ds['id'][i] for i in similar_indices[1:]], conn)

        conn.close()
        # print("_train", flattened)
