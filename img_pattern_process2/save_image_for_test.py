...
from img_pattern_process2  import dbJob
from img_pattern_process2 import common
...
tf.app.flags.DEFINE_string('train_directory', '/data/www/oneten/dl_img_pattern_process2/test_data_dir/',
                           'Training data directory')
tf.app.flags.DEFINE_string('validation_directory', '/data/www/oneten/dl_img_pattern_process2/test_data_dir/',
                           'Validation data directory')
tf.app.flags.DEFINE_string('output_directory', '/data/www/oneten/dl_img_pattern_process2/test_data_dir',
...
tf.app.flags.DEFINE_string('labels_file', '/data/www/oneten/dl_img_pattern_process2/test_data_dir/labels.txt', 'Labels file')
...
def _make_data(directory, labels_file, i_style_no):
    # shutil.rmtree(directory)
    common.mk_dir_recursive(directory)

    cate_nms, cate_nos = dbJob.getColorNms()
    print(cate_nms)
    ...
    for (i, cate_no) in zip(cate_nms, cate_nos):
        ...
        if i_style_no == 0:
            img_list, file_list = dbJob.getColorPrdImgListForTest(cate_no)
            for url, filename in zip(img_list, file_list):
                ...
                if not os.path.exists(directory+ i +  "/"  + filename):
                    try:
                        ...
                        print(e)
                        print('SKIPPED: urllib.request.urlretrieve eror while dowwnload %s.' % filename)
                        continue
        elif i_style_no > 0:
            img_list, file_list = dbJob.getColorPrdImgListForTest(cate_no)
            for url, filename in zip(img_list, file_list):
...
def main(unused_argv):
  ...
  i_style_no = 0
  _make_data(FLAGS.validation_directory, FLAGS.labels_file, i_style_no)
  _process_dataset('train-cifar', FLAGS.train_directory,
