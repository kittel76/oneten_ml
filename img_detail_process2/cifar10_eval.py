...
tf.app.flags.DEFINE_string('eval_dir', '/data/www/oneten/dl_img_detail_process2/eval', 'Directory where to write event logs.')
...
def eval_once(saver, summary_writer, top_k_op, summary_op, mode):
  ...
  with tf.Session() as sess:
    ...
    try:
      ...
      if mode == 'train':
        log_util.update_job_log(job_code="DETAIL2", accuracy_train=precision, traing_step=int(global_step))
      if mode == 'test':
        log_util.update_job_log(job_code="DETAIL2", accuracy_test=precision, traing_step=int(global_step))
