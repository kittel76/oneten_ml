...
from img_detail_process2 import dbJob
from img_detail_process2 import common
...
def _process_image_files_batch(coder, thread_index, ranges, name, filenames,
  ...
  for s in range(num_shards_per_batch):
    ...
    output_file = os.path.join(FLAGS.output_directory_for_test, output_filename)
    ...
    for i in files_in_shard:
      ...
      if  (width, height) == (FLAGS.src_image_width,FLAGS.src_image_width):
          w.write(out)
          shard_counter += 1
          counter += 1
...
def _make_data(directory, labels_file):
    ...
    cate_nms, cate_nos = dbJob.getStyleNms()
    print(cate_nms)
    ...
    for (i, cate_no) in zip(cate_nms, cate_nos):
        ...
        img_list, file_list = dbJob.getStylePrdImgListForTest(cate_no)
...
def main(unused_argv):
  ...
  print('Saving results to %s' % FLAGS.output_directory_for_test)
  start = time.time()

  _make_data(FLAGS.validation_directory_for_test, FLAGS.labels_file_for_test)
  _process_dataset('train-cifar', FLAGS.train_directory_for_test,
                   FLAGS.train_shards, FLAGS.labels_file_for_test)
