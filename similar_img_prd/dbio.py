...
def truncate_related_prd_list_tmp():
    ...
    curs1 = conn.cursor()
    curs1.execute("truncate table batch_dl_rel_prd_img_d")
    conn.commit()
...
def truncate_related_prd_list():
    ...
    conn = cx_Oracle.connect("oneten", "oneten2017#!", cx_Oracle.makedsn(dbUrl, dbPort, "oraOT1"))
    curs1 = conn.cursor()
    curs1.execute("truncate table dl_rel_prd_img_d")
    conn.commit()
...
def insert_related_prd_list_tmp(prd, item, conn):
    ...
    for i in range(0, data_len):
        tmp = " into batch_dl_rel_prd_img_d (ot_prd_no, sort_no,  rel_prd_no, reg_dtm, regr) values ( " + str(prd) + "," +  str(i+1) + "," + str(item[i]) + ", sysdate, 10000002 ) "
        sql += tmp
        # if i != (data_len - 1):
        #     sql += ","
...
def insert_related_prd_list():
    ...
    conn = cx_Oracle.connect("oneten", "oneten2017#!", cx_Oracle.makedsn(dbUrl, dbPort, "oraOT1"))
    curs = conn.cursor()
    sql = "insert into dl_rel_prd_img_d  \
            select * from batch_dl_rel_prd_img_d "
