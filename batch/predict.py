...
def get_data():
    df = dataio.read_process("/data/www/oneten/dl_data/click.csv", sep=",")
    rows = len(df)
    df = df.iloc[np.random.permutation(rows)].reset_index(drop=True)
    return df

def get_unique_item():
    return np.loadtxt("/data/www/oneten/dl_prd_rating/data_dir/item.csv", delimiter=',', dtype=np.int32)


def get_unique_user():
    return np.loadtxt("/data/www/oneten/dl_prd_rating/data_dir/user.csv", delimiter=',', dtype=np.int32)
...
def getItemRank(target_users, limit_size):
    ...
    with tf.Session() as sess:
        sess.run(init_op)
        initial_step = 0
        ckpt = tf.train.get_checkpoint_state("/data/www/oneten/dl_prd_rating/train_dir")
