...
def main(targetDate):
    ...
    print(items_df["idx_ot_prd_no"].get_value(0))

    for i in xrange(items_df.__len__()):
        temp = [0] * len(items_df)
        temp[i] = 1.0
        itemTrX.append(temp)
    ...
    with tf.Session() as sess:
        ...
        print("itemTrX", itemTrX[0])

        cur_w_var = cur_w_var.assign(cur_w)
        cur_vb_var = cur_vb_var.assign(cur_vb)
        cur_hb_val = cur_hb_val.assign(cur_hb)

        rec = sess.run(v1, feed_dict={v0: itemTrX, W: cur_w_var.eval(), vb: cur_vb_var.eval(), hb: cur_hb_val.eval()})
        ...
        dbio.truncate_related_prd_rank_tmp()
        targetUserCnt = 0
        rec_cnt = 10
        for idx, val in enumerate(rec):
            ...
            otPrdNo = items_df["idx_ot_prd_no"].get(idx)

            item = scored_movies_df.sort_values(["rating"], ascending=False).head(rec_cnt)
            dbio.insert_related_prd_rank_tmp(otPrdNo, item["idx_ot_prd_no"].values, conn)
